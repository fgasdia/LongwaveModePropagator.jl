...
function modeconversion(previous_wavefields::Wavefields{T},
    ...
    @assert LWMS.numheights(previous_wavefields) == LWMS.numheights(wavefields)
    zs = LWMS.heights(wavefields)

    # TODO: Assuming `length(zs)` is always the same, we can reuse `product` b/t calls
    product = Vector{ComplexF64}(undef, LWMS.numheights(wavefields))

    modes = LWMS.eigenangles(wavefields)
    adjmodes = LWMS.eigenangles(adjwavefields)
    prevmodes = LWMS.eigenangles(previous_wavefields)
    ...
    for m in eachindex(modes)
        ...
        N = LWMS.integrate(zs, product, LWMS.RombergEven(ROMBERG_ACCURACY))
        invN[m] = inv(N)
    end
    ...
    for k in eachindex(prevmodes)
        for m in eachindex(adjmodes)
            ...
            I = LWMS.integrate(zs, product, LWMS.RombergEven(ROMBERG_ACCURACY))
