...
function Efield(waveguide::SegmentedWaveguide, wavefields_vec, adjwavefields_vec,
    ...
    maxX = maximum(X)
    Xlength = length(X)
    E = Matrix{ComplexF64}(undef, 3, Xlength)
    ...
    rcvrfields = Matrix{ComplexF64}(undef, 3, 0)  # fields at receiver location
