...
const NUMRXTERMS = 3  # see `modeterms`
...
"Order of `rxterm` field returned by `modeterms`. Also the row number of multi-component `E` field matrix." 
...
function Efield(waveguide::SegmentedWaveguide, wavefields_vec, adjwavefields_vec,
    ...
    maxX = maximum(X)
    Xlength = length(X)
    E = Matrix{ComplexF64}(undef, NUMRXTERMS, Xlength)
    ...
    previous_xmtrfields = Vector{ComplexF64}(undef, 0)  # fields saved from previous segment
    ...
    for j = 1:J  # index of waveguide
        ...
        xmtrfields = Vector{ComplexF64}(undef, N)  # fields generated by transmitter
        rcvrfields = Matrix{ComplexF64}(undef, NUMRXTERMS, N)  # fields at receiver location
        ...
        while X[i] < segment_end
            ...
            totalfield = zeros(eltype(E), NUMRXTERMS)
            for n = 1:N
