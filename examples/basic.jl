...
#
# This example walks through the basic usage of LongwaveModePropagator.jl
# when interfacing with the package through Julia.
#
# The simplest way to `propagate`, the primary function exported by
# LongwaveModePropagator.jl is
#
...
# forms: as a temporal frequency, angular frequency, wave number, or wavelength.
# These are always defined in vacuum.
#
# For consistency and convenience, LongwaveModePropagator.jl defines a `Frequency`
...
# ### Antennas
#
# LongwaveModePropagator.jl can compute the far field in the waveguide produced from
# energized small dipole antennas of any orientation in the waveguide.
#
# All antennas are subtypes of the abstract `Antenna` type.
# Currently all antennas are also subtypes of the `AbstractDipole` type.
#
# The most general antenna is the `Dipole`, which has `azimuth_angle` and
...
# There are also two special cases of `Dipole`: `VerticalDipole` and `HorizontalDipole`.
# The `VerticalDipole` struct has no fields because for our purposes, it
...
# The `HorizontalDipole` struct has the `azimuth_angle` field in radians to describe its
# orientation relative to the waveguide.
...
# dispatch capability to avoid calculating field terms that are unnecessary
# for particularly oriented emitters (because they are 0 or 1).
#
# ### [Emitters](@id emitters_intro)
#
# Emitters emit radiofrequency energy in the waveguide.
# These are typically large, fixed VLF transmitters on the ground, but they could also be
# airborne.
# The hierarchy in LongwaveModePropagator.jl was designed with the thought that this
...
#
# All emitters subtype the abstract `Emitter` type.
#
# The `Transmitter` struct is used to define a typical man-made transmitter
# on the ground.
...
# `AirborneTransmitter` defined that adds an `altitude` field to the basic
# `Transmitter` object. Further implementation and verification of the
...
#
# Structs that subtype `AbstractSampler` probe the fields in the waveguide generated
# by an emitter.
...
# The general `Sampler` struct specifies
#
...
#
# There is a special `GroundSampler` which lacks the `altitude` field in `Sampler`s
...
#
# The abstract `Waveguide` type is used to describe the earth-ionosphere waveguide
# with a `BField`, `Species`, and `Ground`.
#
...
#
# This is the case when modeling with only a single homogeneous waveguide along
# the propagation path.
#
# To model an inhomogeneous propagation path, there is a `SegmentedWaveguide` type.
...
# ### BFields
#
# The `BField` type describes the magnitude and direction of the background
# magnetic field.
...
# `dip` angle in radians from the horizontal positive when directed into Earth,
...
# ### Species
#
# The `Species` struct identifies a constituent species of the ionosphere.
...
#
# `numberdensity` and `collisionfrequency` should be "callables" (functions) that
...
# height cutoff and threshold arguments.
#
# `electroncollisionfrequency` is a function of height `z` only, so it is not
# necessary to generate a lambda function and we can pass the function handle
# directly.
#
# ### Ground
#
...
# The `propagate` function returns a tuple of complex electric field,
...
#
# As previously mentioned, a `SegmentedWaveguide` contains a vector of
# `HomogeneousWaveguide`s.
...
#
# The first `HomogeneousWaveguide` must have a distance of 0.
...
# We can `propagate` just as before
...
# is a different ionosphere in this second scenario.
